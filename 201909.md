...
[1. 貨物(出荷)データの基幹システム連携(SFTP経由)]
[1]
	FTP はファイル転送プロトコル（File Transfer Protocol）、
	SFTP は SSH（Secure Shell）を使用したファイル転送プロトコル
	（SSH File Transfer Protocol）を表します。
	いずれもインターネット経由でユーザーがファイルのダウンロード
	またはアップロードを行えるようにする一般的な方法です。
	SFTP では、暗号化によりデータが安全に転送されます
	（クリアテキストのデータは転送されません）。
	SFTP か FTP のいずれかを選択する際は、
	より安全なファイル転送方法である SFTP を選ぶことを強くおすすめします。
//これ以降マジで意味不明  
	なお、sftpコマンドは「SSH上で動くアプリケーション」という位置付けのため、
	sshの設定ファイル「/etc/ssh/sshd_config」での設定は必要ですが、
	別途FTPサーバーを稼働させる必要はありません。加えて、同様の理由から、
	RFCのような仕様標準化のための文書も存在しません。
	使用するポートもSSHサーバー（一般的には22番）と同じです。
	類似の機能を持つコマンドとして、SSHを用いてファイルコピーを
	行う「scp」があります。
	scpコマンドは転送速度でsftpより有利とされており、
	バッチ処理に適していますが、中断したファイルコピーを
	再開する機能がありません。sftpでは、ファイル転送が中断した場合に
	再転送が行われます。
	
	[2]
	ネットワーク経由でファイルを転送する方法(sftp)
	ネットワーク経由でファイルを転送する時に広く使われるのが 
	ftpというプロ トコル(通信する時の規則)です。
	sftpでファイルを転送するためには、
	sftpに対応したアプリケーションを利用する必要があります。
	sftpの利用できるWindows用のアプリケーションには WinSCPやPutty があります。

	
	
[まとめ]
ネットワーク経由でファイルを転送する方法がsftp(より安全)で
それを使用するにはsftpに対応したアプリケーションを利用する必要がある。
貨物データの基幹システムとの連携をどのように行うか。
考察：貨物情報ヘッダと貨物情報明細が更新されるたびに
Javaで基幹システムにデータを送る？


SSH(secure shell) ：ファイル転送などの通信を安全に行うためのプロトコル
プロトコル:ネットワークでコンピュータが使う言語のようなもの。手順、規約、約束事。
